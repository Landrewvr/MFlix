 const movie_mocks = [
	...
	const find = async () => {
		try {
		const result = {data: movie_mocks, count: movie_mocks.length}
			console.log(result)
			return result;
	...
	const findByGenre = async (genre) => {
		try {
			const filteredArray = movie_mocks.filter(mv => {
				return mv.genres.indexOf(genre) > -1
			});
			const result = {data: filteredArray , count: filteredArray.length};
			return result;
		} catch (err) {
			throw err;
		}
